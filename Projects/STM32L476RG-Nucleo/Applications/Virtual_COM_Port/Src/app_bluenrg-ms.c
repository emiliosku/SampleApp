...
static void User_Process(void)
{
  ...
  /* Read uart header */
  status = HAL_UART_Receive(&hComHandle[COM1], (uint8_t *)uart_header, UARTHEADERSIZE, BLE_UART_SHORT_TIMEOUT);
  ...
  if (status == HAL_OK) {
    ...
    if (len > 0) {
      ...
      if(HAL_UART_Receive(&hComHandle[COM1], (uint8_t *)aRxBuffer, len, BLE_UART_LONG_TIMEOUT) != HAL_OK)
      {
        Error_Handler();
      }
    }
  ...
  while (HAL_GPIO_ReadPin(HCI_TL_SPI_EXTI_PORT, HCI_TL_SPI_EXTI_PIN) == GPIO_PIN_SET) {
    ...
    if (rx_bytes > 0) {
      ...
      for (i = 0; i < rx_bytes; i++) {
        ...
        if(HAL_UART_Transmit(&hComHandle[COM1], (uint8_t*)&rx_buffer[i], 1, BLE_UART_LONG_TIMEOUT)!= HAL_OK) {
          Error_Handler();
        }         
      }
      PRINTF("\n");
...
void handle_vendor_command(uint8_t* cmd, uint8_t cmd_len)
{
  ...
  if(HAL_UART_Transmit(&hComHandle[COM1], (uint8_t*)response, len, BLE_UART_LONG_TIMEOUT) != HAL_OK) {
    Error_Handler();
  }
}
